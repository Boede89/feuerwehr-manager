(function () {
  ...
  function listReturnPath() {
    return '/berichte?tab=geraetewart&year=' + filters.year;
  }

  function tablePdfPrintActions(reportId) {
    var base = '/berichte/geraetewartmitteilungen/' + reportId;
  ...
  function buildFooter(meta) {
    ...
    html += '<a class="btn btn--outline" href="/berichte/geraetewartmitteilungen/' + meta.reportId +
      '/pdf?unit=' + encodeURIComponent(unitId) + '">PDF herunterladen</a>';
    html += '<button type="button" class="btn btn--outline" id="btn-gwm-modal-print">Drucken</button>';
    ...
    document.getElementById('btn-gwm-modal-print')?.addEventListener('click', function () {
      postAction('/berichte/geraetewartmitteilungen/' + meta.reportId + '/drucken', returnPath);
    });
  }
  ...
  function loadList() {
    return fetch('/berichte/geraetewartmitteilungen/list?unit=' + encodeURIComponent(unitId) +
