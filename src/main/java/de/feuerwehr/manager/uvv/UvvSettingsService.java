...
public class UvvSettingsService {

    private static final TypeReference<List<Long>> LONG_LIST = new TypeReference<>() {};

    private final UnitRepository unitRepository;
    private final UnitUvvSettingsRepository settingsRepository;
    private final UserRepository userRepository;
    private final ObjectMapper objectMapper;

    @Transactional
    public UnitUvvSettings ensureSettings(long unitId) {
        return settingsRepository.findByUnitId(unitId).orElseGet(() -> {
            Unit unit = unitRepository
                    .findById(unitId)
                    .orElseThrow(() -> new IllegalArgumentException("Einheit nicht gefunden."));
            UnitUvvSettings settings = new UnitUvvSettings();
    ...
    public UnitUvvSettings save(
        ...
        if (intervalMonths < 1 || intervalMonths > 36) {
            throw new IllegalArgumentException("Das Intervall muss zwischen 1 und 36 Monaten liegen.");
        }
        if (warnDays < 0 || warnDays > 365) {
            throw new IllegalArgumentException("Die Warnzeit muss zwischen 0 und 365 Tagen liegen.");
        }
        UnitUvvSettings settings = ensureSettings(unitId);
    ...
    @Transactional(readOnly = true)
    public List<Long> parseNotificationUserIds(UnitUvvSettings settings) {
