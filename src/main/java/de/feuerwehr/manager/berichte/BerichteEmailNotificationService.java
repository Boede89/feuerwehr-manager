...
public class BerichteEmailNotificationService {
    ...
    public void onBerichteEmail(BerichteEmailEvent event) {
        if (event == null) {
            return;
        }
        switch (event.trigger()) {
            case CREATE -> {
                if (event.reportType() == BerichteEmailReportType.CHECKLISTEN && event.vehicleId() != null) {
                    trySendChecklistCreated(event.unitId(), event.vehicleId(), event.reportId());
                } else {
                    trySendOnCreate(event.unitId(), event.reportType(), event.reportId());
                }
            }
