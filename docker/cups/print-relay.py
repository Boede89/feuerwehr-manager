...
PRINTER_LINE = re.compile(r"^printer\s+(\S+)\s+is\s+", re.IGNORECASE)
...
class PrintHandler(BaseHTTPRequestHandler):
    ...
    def handle_printers(self):
        ...
        for line in proc.stdout.splitlines():
            match = PRINTER_LINE.match(line.strip())
            if match:
                printers.append(match.group(1))
        body = json.dumps({"printers": printers, "default": DEFAULT_PRINTER}).encode("utf-8")
